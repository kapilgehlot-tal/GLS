...
function getGuideData(guideData) {
  chrome.storage.sync.get(["step"], (result) => {
    ...
    $(action.selector).after(template);
    ...
    $(".popover-content").html(contents["#content"]);
...
function prevStep() {
  ...
  const nextStep = action?.contents;

  if (action?.roleTexts?.nextBt) {
    $(".next-btn").text(action?.roleTexts?.nextBt);
  } else {
    $(".next-btn").text("Next");
  }
...
function common() {
  ...
  if (!Object.keys($(action.selector)).length) {
    ...
    $(action.selector).last().after(template);
  }
